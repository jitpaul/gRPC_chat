...
class ChatClient {
	...
	StreamRequest createChatRequest(int type, std::string messageNew = "") {
        StreamRequest n;
	    if(type==1){  //Start Sesssion Request
	    ...
	    else if(type==2){ //Join Sesssion Request
        ...
        else if(type==3){ //New Message Request
	...
	void parse(std::string msg, std::vector<std::string>& ret){
		...
		std::string temp2;
		while(std::getline(temp,temp2,' ')) ret.push_back(temp2);
	}
	...
	void makeRequests(std::shared_ptr<ClientReaderWriter<StreamRequest, StreamResponse> > stream){
		...
		while(true){
			std::getline(std::cin,msg);
			if(msg=="quit") {stream->Write(createChatRequest(4));break;}
	...
	void processResponses(std::shared_ptr<ClientReaderWriter<StreamRequest, StreamResponse> > stream){
        ...
        while (stream->Read(&serverResponse)) {
		    if(serverResponse.has_startsession_response()){ //Start Session Response
			    session_id = serverResponse.startsession_response().session_id();
				std::cout << "    Your session_id is "<<session_id<<" \n";
			}
			else if(serverResponse.has_joinsession_response()) {//Join Session Response
				session_join_flag = false;
				std::cout <<"     Could not connect to the session. " << serverResponse.joinsession_response().message()<<" Try again.\n";
			}
            else if(serverResponse.has_login_response()){ //User Login Notification Response
			    if(serverResponse.login_response().username()==userName) std::cout <<"    " <<"You are now logged in.\n";
				else std::cout <<"    " << serverResponse.login_response().username()<<" has logged in.\n";
			}
	        else if(serverResponse.has_logout_response()){ //User Logout Notification Response
		        std::cout <<"    " << serverResponse.logout_response().username()<<" has logged out.\n";
			}
	        else if(serverResponse.has_message_response()){ //New Message Notification Response
    ...
    void RouteChat() {
        ...
        if (status.ok()) std::cout << "You have successfully logged out." << std::endl;
		else std::cout << "There was a server error." << std::endl;
    } 
